...
def setup_distributed(rank, world_size):
    ...
    print(f"[Rank {rank}] Process group initialized, device set to cuda:{rank}", flush=True)
...
def run(rank, world_size, hps):
    ...
    if is_distributed:
        print(f"[Rank {rank}] Wrapping models with DDP...", flush=True)
        net_g = DDP(net_g, device_ids=[rank], find_unused_parameters=False)
        net_d = DDP(net_d, device_ids=[rank])
        print(f"[Rank {rank}] DDP wrapping complete", flush=True)
...
def train_epoch(rank, epoch, hps, net_g, net_d, optim_g, optim_d,
    ...
    if is_main_process:
        print(f"[Rank {rank}] Starting epoch {epoch}, total batches: {len(train_loader)}", flush=True)

    for batch_idx, batch in enumerate(train_loader):
        if batch_idx == 0 and is_main_process:
            print(f"[Rank {rank}] Processing first batch...", flush=True)
        ...
        if global_step % hps.train.log_interval == 0 and is_main_process:
            print(f"[Rank {rank}] Step {global_step} completed", flush=True)
            logger.info(
